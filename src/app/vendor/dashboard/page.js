...
const ALL_FIELDS = [
  ...
  { key: 'pliName', label: 'PLI Name' },
  { key: 'requestDate', label: 'Requested Date' },
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setUser(parsed);
        fetchRequests(parsed.vendorCode);
      }
    } catch {}
  }, []);

  const fetchRequests = (vendorCode) => {
    fetch(`/api/vendor/pli?vendorCode=${vendorCode}`).then(res => res.json()).then(data => setRequests(data.requests || [])).catch(console.error);
  };

  const handleSearch = () => {
    if (!user) return;
    let url = `/api/vendor/pli?vendorCode=${user.vendorCode}`;
    if (filters.plant) url += `&plant=${filters.plant}`;
    if (filters.pliName) url += `&pliName=${filters.pliName}`;
    if (filters.status) url += `&status=${filters.status}`;
    fetch(url).then(res => res.json()).then(data => setRequests(data.requests || []));
  };
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4 animate-slide-down">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <select value={filters.plant} onChange={e => setFilters({ ...filters, plant: e.target.value })} className="px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-600">
              <option value="">Plant</option><option>1900</option><option>5100</option><option>1003</option>
            </select>
            ...
            <select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })} className="px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-600">
              <option value="">Select Status</option><option>Pending Submission</option><option>Submitted</option><option>Closed</option><option>Rejected</option>
            </select>
            <div className="flex gap-2 items-center">
              ...
              <button onClick={() => { setFilters({ plant: '', noOfItems: '', pliName: '', requestDate: '', status: '' }); if (user) fetchRequests(user.vendorCode); }} className="px-4 py-2 text-sm font-medium text-red-500">Reset</button>
            </div>
          </div>
        </div>
      )}
