...
import { Filter, ChevronRight, ChevronDown, Pencil, Trash2, Download, CheckCircle, XCircle, ListFilter, FileSpreadsheet, ArrowLeft, ChevronLeft, ChevronsLeft, ChevronsRight, Settings2 } from 'lucide-react';
...
export default function BuyerPLIDashboard() {
  ...
  const handleExcelUpload = async (file) => {
    try {
      ...
      if (rows.length === 0) { showNotif('Excel file is empty.','error'); return; }
      const headers = Object.keys(rows[0]);
      const missing = REQUIRED_COLUMNS.filter(col => !headers.includes(col));
      if (missing.length > 0) { showNotif(`Missing columns: ${missing.join(', ')}`,'error'); return; }
      let hasError = false;
      ...
      const newRequests = [];
      let counter = allRequests.length + 1;
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      {notification && (<div className={`fixed top-4 right-4 z-50 max-w-md px-5 py-3 rounded-xl shadow-lg text-sm font-medium animate-fade-in ${notification.type==='error'?'bg-red-50 text-red-700 border border-red-200':'bg-green-50 text-green-700 border border-green-200'}`}>{notification.message}<button onClick={()=>setNotification(null)} className="ml-3 opacity-50 hover:opacity-100">✕</button></div>)}
